...
// last saved: <2025-July-07 20:35:14>
...
const edgejs = require("apigee-edge-js"),
  ...
  getopt = new Getopt([
    [
      "",
      "sacreds=ARG",
      "Required. the .json service account credentials the proxy will use to read the gsheet.",
    ],
...
function getNewGoogleapisToken(oAuth2Client, tokenStashPath, cb) {
  ...
  new Promise((resolve) => setTimeout(resolve, 4200)).then(() => {
...
function oauth2Authorize(credentials, cb) {
  ...
  const tokenStashPath = path.resolve(
    dataDir,
    ".gsheets_abac_token_stash.json",
  );
...
fsPromise.readFile(opt.options.sacreds, "utf8").then((saCreds) => {
  if (saCreds) {
    saCreds = JSON.parse(saCreds);
  }
  if (!saCreds || !saCreds.project_id || !saCreds.client_email) {
    return Promise.reject(
      new Error("Cannot read the Service Account creds file."),
    );
  }
    ...
    sheets.spreadsheets.create(request, function (e, createResponse) {
      ...
      sheets.spreadsheets.values.update(update1, (e, updateResponse) => {
        ...
        sheets.spreadsheets.batchUpdate(batch, function (e, _) {
          ...
          common.logWrite(
            `Share that sheet with this email: ${saCreds.client_email}`,
          );
